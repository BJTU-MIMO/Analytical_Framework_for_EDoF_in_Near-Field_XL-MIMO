function [EDoF_UPA_Patch,Numerator,Denominator] = functionComputerEDoF_UPA_Patch_Dyadic_average(rr,rt,M,N,Ath,Atv,Arh,Arv,wavenumber)

%%=============================================================
%The file is used to compute the simulation results for the EDoF for the
%UPA-based system with patch antennas over dyadic channels of the paper:
%
                    ...
                    for q = 1:3
                        ...
                        for n = 1:N
                            for nn = 1:5
                                ...
                                G_Dyadic_rt = functionDyadicGreen(r_nn(:,nn),r_mm(:,m1),wavenumber);
                                G_Dyadic_rtt = functionDyadicGreen(r_nn(:,nn),r_mmm(:,m2),wavenumber);

                                Numerator = Numerator + abs(G_Dyadic_rt(p,q))^2/(5*M); %Redundant iterations for mm

                                for i = 1:3

                                    Denominator1 = Denominator1 + G_Dyadic_rt(i,p)'*G_Dyadic_rtt(i,q);
                                end
                            end
                        end
